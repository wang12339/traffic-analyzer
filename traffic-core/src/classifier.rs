...
static RULES: &[Rule] = &[
    ...
    Rule(
        ...
        "Google Maps",
        "Navigation",
        &["maps.google.com", "googleapis.com"],
    ),
    // ── Browser ──
    ...
    Rule(
        ...
        "HBO Max",
        "Video",
        &["hbomax.com", "hbomaxcdn.com", "max.com", "wbd.com"],
    ),
    ...
    Rule(
        ...
        "Riot Games",
        "Game",
        &["riotgames.com", "lolstatic.com", "pvp.net"],
    ),
    ...
    Rule(
        ...
        "Minecraft",
        "Game",
        &["minecraft.net", "minecraftforge.net", "mojang.com"],
    ),
    Rule(
        228,
        "Unity",
        "Game",
        &["unity.com", "unity3d.com", "unityads.com"],
    ),
    // ── More Social ──
    Rule(
        230,
    ...
    Rule(
        ...
        "Stability AI",
        "AI",
        &["stability.ai", "stabilityusercontent.com"],
    ),
    ...
    Rule(
        ...
        "Replicate",
        "AI",
        &["replicate.com", "replicateusercontent.com"],
    ),
    // ── Productivity / Office ──
    Rule(
        250,
    ...
    Rule(
        ...
        "Linear",
        "Productivity",
        &["linear.app", "linearusercontent.com"],
    ),
    // ── Storage ──
    Rule(
        260,
        "Google Drive",
        "Storage",
        &["drive.google.com", "googleusercontent.com", "ggpht.com"],
    ),
    Rule(
        261,
        "Dropbox",
    ...
    Rule(
        ...
        "iCloud",
        "Storage",
        &["icloud.com", "icloud-content.com", "icloud.cdn"],
    ),
    ...
    Rule(
        ...
        "Philips Hue",
        "IoT",
        &["meethue.com", "philips-hue.com"],
    ),
    Rule(
        281,
        "TP-Link Tapo",
        "IoT",
        &["tplinkcloud.com", "tapologic.com"],
    ),
    ...
    Rule(
        ...
        "Home Assistant",
        "IoT",
        &["home-assistant.io", "nabucasa.com"],
    ),
    ...
    Rule(
        ...
        "GitHub Actions",
        "Developer",
        &["actions.githubusercontent.com", "githubusercontent.com"],
    ),
    Rule(
        294,
        "Cloudflare Workers",
    ...
    if !sni.is_empty() || !dns.is_empty() {
        ...
        for rule in RULES {
            if rule
                .3
                .iter()
                .any(|p| domain_matches(p, &sni_lower) || domain_matches(p, &dns_lower))
            {
...
static JA3_FINGERPRINTS: &[(&str, u32, &str, &str, f32, &str)] = &[
    ...
    (
        ...
        "Malware",
        0.85,
        "Cobalt Strike variant (shared JA3 with Chrome)",
    ),
    (
        "aab1c0bd39f878b0",
        500,
...
fn ja3_engine(ja3: &str, _sni: &str) -> EngineVerdict {
    ...
    if prefix.starts_with("e7") || prefix.starts_with("51") || prefix.starts_with("a0") {
        return EngineVerdict {
            engine: "ja3".into(),
            app_id: 520,
            app_name: "Chrome (推测)".into(),
            app_category: "Browser".into(),
            confidence: 0.30,
...
const ENGINE_WEIGHTS: &[(&str, f32)] = &[("rules", 1.0), ("ja3", 0.7), ("flow", 0.5)];
...
fn weighted_vote(engines: &[EngineVerdict]) -> Classification {
    ...
    let mut votes: std::collections::HashMap<u32, (f32, &EngineVerdict)> =
    ...
    let mut sorted: Vec<(u32, f32, &EngineVerdict)> = votes
        .into_iter()
    ...
    if *winner_score > runner_up_score * 1.5 || *winner_score > 0.5 {
        ...
        Classification::named(*winner_id, &winner_v.app_name, &winner_v.app_category, conf)
    } else {
...
static MAC_OUI: &[(&str, &str, f32)] = &[
    ...
    ("fc:e0:45", "Apple", 0.90),
    // Samsung
    ("00:15:99", "Samsung", 0.85),
    ("08:6a:0a", "Samsung", 0.85),
    ("0c:6e:0f", "Samsung", 0.85),
    ("10:51:63", "Samsung", 0.85),
    ("10:58:dc", "Samsung", 0.85),
    ("14:af:e7", "Samsung", 0.85),
    ("18:3e:28", "Samsung", 0.85),
    ("1c:ed:8c", "Samsung", 0.85),
    ("20:37:06", "Samsung", 0.85),
    ("24:ee:9a", "Samsung", 0.85),
    ("28:6c:07", "Samsung", 0.85),
    ("2c:15:d9", "Samsung", 0.85),
    ("30:68:5c", "Samsung", 0.85),
    ("34:fc:ef", "Samsung", 0.85),
    ("38:08:e1", "Samsung", 0.85),
    ("3c:0e:1a", "Samsung", 0.85),
    ("44:3c:09", "Samsung", 0.85),
    ("4c:0f:6e", "Samsung", 0.85),
    ("50:6f:9a", "Samsung", 0.85),
    ("54:6c:81", "Samsung", 0.85),
    ("58:c3:8d", "Samsung", 0.85),
    ("5c:49:79", "Samsung", 0.85),
    ("60:a4:23", "Samsung", 0.85),
    ("64:20:0c", "Samsung", 0.85),
    ("68:a0:3e", "Samsung", 0.85),
    ("6c:ad:ef", "Samsung", 0.85),
    ("70:4d:7b", "Samsung", 0.85),
    ("74:22:bb", "Samsung", 0.85),
    ("78:14:34", "Samsung", 0.85),
    ("7c:1c:4d", "Samsung", 0.85),
    ("80:15:c5", "Samsung", 0.85),
    ("84:db:2f", "Samsung", 0.85),
    ("88:36:6c", "Samsung", 0.85),
    ("8c:8c:aa", "Samsung", 0.85),
    ("90:3c:b3", "Samsung", 0.85),
    ("94:7b:c9", "Samsung", 0.85),
    ("98:0c:82", "Samsung", 0.85),
    ("9c:4e:8e", "Samsung", 0.85),
    ("a0:0b:ba", "Samsung", 0.85),
    ("a4:77:33", "Samsung", 0.85),
    ("a8:4e:3f", "Samsung", 0.85),
    ("ac:5a:14", "Samsung", 0.85),
    ("b0:6a:10", "Samsung", 0.85),
    ("b4:3e:08", "Samsung", 0.85),
    ("b8:5a:f7", "Samsung", 0.85),
    ("bc:9c:31", "Samsung", 0.85),
    ("c0:99:a4", "Samsung", 0.85),
    ("c4:7c:1a", "Samsung", 0.85),
    ("c8:0c:c8", "Samsung", 0.85),
    ("cc:3d:82", "Samsung", 0.85),
    ("d0:17:12", "Samsung", 0.85),
    ("d4:08:95", "Samsung", 0.85),
    ("d8:10:2b", "Samsung", 0.85),
    ("dc:09:4b", "Samsung", 0.85),
    ("e0:9b:47", "Samsung", 0.85),
    ("e4:6c:79", "Samsung", 0.85),
    ("e8:1c:ba", "Samsung", 0.85),
    ("ec:17:66", "Samsung", 0.85),
    ("f0:4d:a2", "Samsung", 0.85),
    ("f4:7e:4c", "Samsung", 0.85),
    ("f8:e7:1e", "Samsung", 0.85),
    // Xiaomi
    ("aa:80:a0", "Xiaomi", 0.90),
    ("de:2c:28", "Xiaomi", 0.90),
    ("18:2b:c9", "Xiaomi", 0.85),
    ("20:09:d3", "Xiaomi", 0.85),
    ("24:16:6d", "Xiaomi", 0.85),
    ("28:84:fd", "Xiaomi", 0.85),
    ("2c:aa:8e", "Xiaomi", 0.85),
    ("30:4b:a1", "Xiaomi", 0.85),
    ("34:ce:00", "Xiaomi", 0.85),
    ("38:68:dd", "Xiaomi", 0.85),
    ("3c:3f:52", "Xiaomi", 0.85),
    ("40:5e:58", "Xiaomi", 0.85),
    ("44:ef:77", "Xiaomi", 0.85),
    ("48:7a:52", "Xiaomi", 0.85),
    ("4c:49:e3", "Xiaomi", 0.85),
    ("50:ec:8c", "Xiaomi", 0.85),
    ("54:5e:f2", "Xiaomi", 0.85),
    ("58:e6:3f", "Xiaomi", 0.85),
    ("5c:02:6b", "Xiaomi", 0.85),
    ("60:6d:3c", "Xiaomi", 0.85),
    ("64:9a:be", "Xiaomi", 0.85),
    ("68:f8:f7", "Xiaomi", 0.85),
    ("6c:cf:35", "Xiaomi", 0.85),
    ("70:3a:94", "Xiaomi", 0.85),
    ("74:4e:ac", "Xiaomi", 0.85),
    ("78:9e:d0", "Xiaomi", 0.85),
    ("7c:a2:2f", "Xiaomi", 0.85),
    // Huawei
    ("00:18:82", "Huawei", 0.85),
    ("04:bd:70", "Huawei", 0.85),
    ("08:16:d3", "Huawei", 0.85),
    ("0c:1d:af", "Huawei", 0.85),
    ("10:1b:54", "Huawei", 0.85),
    ("14:48:fc", "Huawei", 0.85),
    ("18:2a:7b", "Huawei", 0.85),
    ("20:15:06", "Huawei", 0.85),
    ("24:46:c8", "Huawei", 0.85),
    ("28:6e:d4", "Huawei", 0.85),
    ("2c:1f:23", "Huawei", 0.85),
    ("30:6f:08", "Huawei", 0.85),
    ("34:61:bb", "Huawei", 0.85),
    ("38:59:f8", "Huawei", 0.85),
    ("3c:4e:09", "Huawei", 0.85),
    ("40:1a:5e", "Huawei", 0.85),
    ("44:6e:27", "Huawei", 0.85),
    ("48:22:54", "Huawei", 0.85),
    ("4c:5e:a4", "Huawei", 0.85),
    ("50:3c:fc", "Huawei", 0.85),
    ("54:33:cb", "Huawei", 0.85),
    ("58:17:0c", "Huawei", 0.85),
    ("5c:fc:70", "Huawei", 0.85),
    ("60:90:44", "Huawei", 0.85),
    ("64:16:8f", "Huawei", 0.85),
    ("68:1c:a1", "Huawei", 0.85),
    ("6c:6d:0a", "Huawei", 0.85),
    ("70:5a:8d", "Huawei", 0.85),
    ("74:1f:4a", "Huawei", 0.85),
    ("78:9f:70", "Huawei", 0.85),
    // Google / Nest
    ("00:1a:11", "Google", 0.85),
    ("08:3e:8e", "Google", 0.85),
    ("0c:8c:8d", "Google", 0.85),
    ("10:6f:d9", "Google", 0.85),
    ("14:a6:4b", "Google", 0.85),
    ("18:b4:30", "Google", 0.85),
    ("1c:5a:6b", "Google", 0.85),
    ("24:5f:df", "Google", 0.85),
    ("28:8a:1c", "Google", 0.85),
    ("2c:27:d7", "Google", 0.85),
    ("30:8c:fb", "Google", 0.85),
    ("34:7e:5c", "Google", 0.85),
    ("38:9c:a5", "Google", 0.85),
    ("3c:5e:c3", "Google", 0.85),
    ("40:9d:0b", "Google", 0.85),
    ("44:d9:e7", "Google", 0.85),
    ("48:5a:3f", "Google", 0.85),
    ("4c:3b:74", "Google", 0.85),
    ("50:2f:9b", "Google", 0.85),
    ("54:60:09", "Google", 0.85),
    ("58:cb:52", "Google", 0.85),
    ("5c:8a:38", "Google", 0.85),
    ("60:92:17", "Google", 0.85),
    ("64:a6:51", "Google", 0.85),
    ("68:54:5a", "Google", 0.85),
    ("6c:0b:63", "Google", 0.85),
    ("70:5d:cc", "Google", 0.85),
    // Amazon
    ("00:75:92", "Amazon", 0.85),
    ("04:49:69", "Amazon", 0.85),
    ("08:91:01", "Amazon", 0.85),
    ("0c:5b:8f", "Amazon", 0.85),
    ("10:02:b5", "Amazon", 0.85),
    ("14:a0:2c", "Amazon", 0.85),
    ("18:68:cb", "Amazon", 0.85),
    ("1c:66:aa", "Amazon", 0.85),
    ("20:3e:7a", "Amazon", 0.85),
    ("24:0a:11", "Amazon", 0.85),
    ("28:0a:21", "Amazon", 0.85),
    ("2c:15:59", "Amazon", 0.85),
    ("30:0a:0b", "Amazon", 0.85),
    ("34:1c:f4", "Amazon", 0.85),
    ("38:3f:5c", "Amazon", 0.85),
    ("3c:5f:5c", "Amazon", 0.85),
    ("40:6c:bf", "Amazon", 0.85),
    ("44:b2:ff", "Amazon", 0.85),
    ("48:e7:29", "Amazon", 0.85),
    ("4c:11:ae", "Amazon", 0.85),
    // TP-Link
    ("00:1a:a9", "TP-Link", 0.85),
    ("00:26:b9", "TP-Link", 0.85),
    ("04:f0:21", "TP-Link", 0.85),
    ("08:27:98", "TP-Link", 0.85),
    ("0c:9d:92", "TP-Link", 0.85),
    ("10:2b:0f", "TP-Link", 0.85),
    ("14:cf:92", "TP-Link", 0.85),
    ("18:a6:f7", "TP-Link", 0.85),
    ("1c:3a:4f", "TP-Link", 0.85),
    ("20:1c:e0", "TP-Link", 0.85),
    ("24:05:88", "TP-Link", 0.85),
    ("28:2c:02", "TP-Link", 0.85),
    ("2c:3f:38", "TP-Link", 0.85),
    ("30:b4:9e", "TP-Link", 0.85),
    ("34:6a:2b", "TP-Link", 0.85),
    ("38:5b:2c", "TP-Link", 0.85),
    ("3c:2c:30", "TP-Link", 0.85),
    ("40:16:7a", "TP-Link", 0.85),
    ("44:2c:05", "TP-Link", 0.85),
    ("48:22:57", "TP-Link", 0.85),
    // ASUS
    ("00:1a:92", "ASUS", 0.85),
    ("00:1e:2a", "ASUS", 0.85),
    ("04:92:e3", "ASUS", 0.85),
    ...
    ("3a:a4:28", "Vivo", 0.90),
    // OnePlus
    ("02:00:00", "OnePlus", 0.80),
    ("9a:1c:8c", "OnePlus", 0.80),
    // OPPO
    ("58:03:fb", "OPPO", 0.85),
    ("6c:02:e0", "OPPO", 0.85),
    ("b0:75:d5", "OPPO", 0.85),
    // Lenovo
    ("00:0b:ab", "Lenovo", 0.85),
    ("00:1a:6b", "Lenovo", 0.85),
    ("18:67:b0", "Lenovo", 0.85),
    ("3c:7c:3f", "Lenovo", 0.85),
    ("5c:e8:39", "Lenovo", 0.85),
    // Dell
    ("00:14:22", "Dell", 0.85),
    ("00:1d:09", "Dell", 0.85),
    ("00:21:9b", "Dell", 0.85),
    ...
    ("00:e0:4c", "Realtek", 0.80),
    ("00:e0:61", "Realtek", 0.80),
    // Roku / Streaming
    ("00:0d:4f", "Roku", 0.85),
    ("48:a6:d2", "Roku", 0.85),
    // Sony
    ...
    ("1c:3b:f3", "Sony", 0.85),
    // Nintendo
    ("00:1b:ea", "Nintendo", 0.85),
    ("00:26:be", "Nintendo", 0.85),
    ("04:3d:48", "Nintendo", 0.85),
    ("28:6f:b4", "Nintendo", 0.85),
    ("2c:10:c1", "Nintendo", 0.85),
    ("34:ee:8c", "Nintendo", 0.85),
    // Microsoft / Xbox
    ("00:15:5d", "Microsoft", 0.85),
    ("00:1d:d8", "Microsoft", 0.85),
    ("00:22:48", "Microsoft", 0.85),
    ("14:4f:8a", "Microsoft", 0.85),
    ("48:50:fd", "Microsoft", 0.85),
    ("58:6a:6b", "Microsoft", 0.85),
    // Clash/Stash proxy (randomized MACs, pattern-based)
    ("e2:08:f4", "Clash/Stash", 0.80),
    ("5a:e2:02", "Clash/Stash", 0.80),
    // OpenWrt / LEDE
