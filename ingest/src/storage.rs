...
impl ClickStore {
    pub async fn new(addr: &str, database: &str) -> Result<Self> {
        ...
        Ok(Self { client, http_client: reqwest::Client::builder().timeout(std::time::Duration::from_secs(5)).build()?, database: database.to_string(), ch_http_url })
    }
    ...
    pub async fn write_flows(&self, records: &[traffic_core::FlowRecord]) -> Result<()> {
        ...
        let body = json_rows.iter().map(|j| j.to_string()).collect::<Vec<_>>().join("\n");
    ...
    async fn http_post_retry(&self, url: &str, body: String) -> Result<()> {
        ...
        for (attempt, delay_ms) in delays.iter().enumerate() {
                ...
                .send().await
            {
                Ok(resp) => {
                    ...
                    tracing::warn!("CH write attempt {} failed: {}", attempt + 1, last_err.as_ref().unwrap());
                }
                Err(e) => {
        ...
        Err(anyhow::anyhow!("ClickHouse write failed after {} attempts: {}", delays.len(), last_err.unwrap_or_default()))
    }
...
fn urlencoding(s: &str) -> String {
    ...
    pub async fn write_json_flow(&self, rec: &serde_json::Value) -> Result<(), anyhow::Error> {
        ...
        row.insert("src_port".into(), rec["src_port"].as_i64().unwrap_or(0).into());
        row.insert("dst_port".into(), rec["dst_port"].as_i64().unwrap_or(0).into());
        row.insert("protocol".into(), rec["protocol"].as_str().unwrap_or("TCP").into());
        row.insert("sni".into(), rec["tls_sni"].as_str().unwrap_or("").into());
        row.insert("http_host".into(), rec["http_host"].as_str().unwrap_or("").into());
        row.insert("ja3".into(), "".into());
        row.insert("dns_domain".into(), rec["dns_domain"].as_str().unwrap_or("").into());
        ...
        let (app_id, app_name, app_category, confidence) = (cls.app_id, cls.app_name, cls.app_category, cls.confidence);

        // Device type inference
        let device_mfg = traffic_core::classifier::infer_device(sni, dns, src_mac);

        row.insert("packets_up".into(), rec["packets"].as_i64().unwrap_or(0).into());
        ...
        row.insert("duration_ms".into(), ((ls - fs).max(0) as i64 / 1_000_000).into());
        ...
        row.insert("device_hostname".into(), rec["hostname"].as_str().unwrap_or("").into());
    ...
    pub async fn write_device_info(&self, rec: &serde_json::Value) -> Result<(), anyhow::Error> {
        ...
        let manu2 = if vc.contains("iphone") { "Apple" } else if vc.contains("xiaomi") || vc.contains("mi ") { "Xiaomi" }
            else if vc.contains("huawei") { "Huawei" } else { "" };
        let final_mfg = if !manu2.is_empty() { manu2 } else { &manufacturer };

        let query = format!("INSERT INTO {}.device_info FORMAT JSONEachRow", self.database);
    ...
    pub async fn write_http_session(&self, rec: &serde_json::Value) -> Result<(), anyhow::Error> {
        ...
        if rtype == "http_request" {
            ...
            let query = format!("INSERT INTO {}.http_sessions FORMAT JSONEachRow", self.database);
            let url = format!("{}?database={}&query={}",
                self.ch_http_url, self.database, Self::urlencoding(&query));
...
struct FlowRowCompat<'a> {
    timestamp: &'a str, first_seen: i64, last_seen: i64,
    src_ip: &'a str, dst_ip: &'a str, src_port: u16, dst_port: u16, protocol: &'a str,
    sni: &'a str, ja3: &'a str, dns_domain: &'a str,
    http_host: &'a str, http_method: &'a str, http_ua: &'a str,
    packets_up: u32, packets_down: u32, bytes_up: i64, bytes_down: i64, duration_ms: i64,
    pkt_size_hist: &'a str, pkt_iat_mean_us: f64,
    app_id: u32, app_name: &'a str, app_category: &'a str, confidence: f32,
    src_mac: &'a str, device_manufacturer: &'a str, device_hostname: &'a str,
}
