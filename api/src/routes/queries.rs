...
pub async fn get_stats(state: web::Data<Arc<AppState>>, q: web::Query<TimeQuery>) -> HttpResponse {
    ...
    let sql = format!("SELECT count() as total_flows,sum(bytes_up+bytes_down) as total_bytes,\
        ...
        FROM {}.flows WHERE timestamp >= {}", state.database, se);
    match ch_one::<StatsRow>(&state, &sql).await {
        Ok(r) => HttpResponse::Ok().json(ApiResponse::ok(StatsResponse { total_flows: r.total_flows, total_bytes: r.total_bytes, active_apps: r.apps, unique_devices: r.devices, unique_snis: r.snis, unique_domains: r.domains, flows_per_sec: r.fps })),
...
pub async fn get_flows(state: web::Data<Arc<AppState>>, q: web::Query<FlowQuery>) -> HttpResponse {
    ...
    if let Some(ref ip) = q.search_ip { if !ip.is_empty() { cond.push_str(&format!(" AND (src_ip LIKE '%{}%' OR dst_ip LIKE '%{}%')", ip, ip)); }}
    if let Some(ref d) = q.search_domain { if !d.is_empty() { cond.push_str(&format!(" AND (sni LIKE '%{}%' OR dns_domain LIKE '%{}%')", d, d)); }}
    if let Some(a) = q.app_id { if a > 0 { cond.push_str(&format!(" AND app_id={}", a)); }}
    let sql = format!("SELECT timestamp,src_ip,dst_ip,src_port,dst_port,protocol,\
        sni,dns_domain,app_name,app_category,confidence,\
        bytes_up,bytes_down,packets_up,packets_down,duration_ms,src_mac \
        FROM {}.flows WHERE {} ORDER BY timestamp DESC LIMIT {}", state.database, cond, limit);
...
pub async fn export_csv(state: web::Data<Arc<AppState>>, query: web::Query<FlowQuery>) -> HttpResponse {
    ...
    let mut cond = format!("timestamp >= {}", se);
    if let Some(ref ip) = query.search_ip { if !ip.is_empty() { cond.push_str(&format!(" AND (src_ip LIKE '%{}%' OR dst_ip LIKE '%{}%')", ip, ip)); }}
    if let Some(ref d) = query.search_domain { if !d.is_empty() { cond.push_str(&format!(" AND (sni LIKE '%{}%' OR dns_domain LIKE '%{}%')", d, d)); }}
    if let Some(a) = query.app_id { if a > 0 { cond.push_str(&format!(" AND app_id={}", a)); }}
    let sql = format!("SELECT timestamp,src_ip,dst_ip,app_name,sni,dns_domain,bytes_up+bytes_down as bytes,protocol         FROM {}.flows WHERE {} ORDER BY timestamp DESC LIMIT 10000", state.database, cond);
...
pub async fn get_summary(state: web::Data<Arc<AppState>>) -> HttpResponse {
    ...
    let sql_h = format!("SELECT toHour(timestamp) as h,count() as c FROM {}.flows         WHERE timestamp>=now()-toIntervalDay(1) GROUP BY h ORDER BY c DESC LIMIT 3", state.database);
