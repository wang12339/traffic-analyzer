...
fn main() -> Result<()> {
    ...
    let args = Args::parse();
    info!("Agent starting — iface={}, ingest={}", args.interface, args.ingest_addr);

    // ─── Raw socket (AF_PACKET) ───
    let sock = unsafe {
        let fd = libc::socket(libc::AF_PACKET, libc::SOCK_RAW, (0x0003u16).to_be() as i32);
        if fd < 0 { anyhow::bail!("socket(AF_PACKET) failed: {}", std::io::Error::last_os_error()); }
        fd
    };
    ...
    let sll = sockaddr_ll {
        ...
        sll_ifindex: ifindex,
        sll_hatype: 0, sll_pkttype: 0, sll_halen: 0, sll_addr: [0u8; 8],
    };
    let bind_ret = unsafe {
        libc::bind(sock, &sll as *const _ as *const libc::sockaddr, std::mem::size_of::<sockaddr_ll>() as u32)
    };
    if bind_ret < 0 { anyhow::bail!("bind failed: {}", std::io::Error::last_os_error()); }
    ...
    std::thread::spawn(move || {
        ...
        unsafe {
            libc::setsockopt(sock, libc::SOL_SOCKET, libc::SO_RCVTIMEO, &tv as *const _ as *const libc::c_void,
                             std::mem::size_of::<libc::timeval>() as u32);
        }
        info!("Capture loop started on {}", iface_name);
        while run_cap.load(Ordering::SeqCst) {
            let n = unsafe {
                libc::recvfrom(sock, buf.as_mut_ptr() as *mut libc::c_void, buf.len(), 0, std::ptr::null_mut(), std::ptr::null_mut())
            };
            if n < 0 {
                let err = std::io::Error::last_os_error();
                if err.kind() == std::io::ErrorKind::WouldBlock || err.raw_os_error() == Some(libc::EINTR) {
                    continue;
                }
        ...
        while running.load(Ordering::SeqCst) {
            if stream.is_none() {
                match TcpStream::connect(&args.ingest_addr).await {
                    Ok(s) => { info!("Connected to ingest"); stream = Some(s); }
                    Err(e) => { warn!("Connect failed: {} (retry {:?})", e, RECONNECT_DELAY);
                        tokio::time::sleep(RECONNECT_DELAY).await; continue; }
                }
            }
            ...
            while let Ok(frame) = rx.try_recv() {
                batch.push(frame);
                if batch.len() >= SEND_BUF_SIZE { break; }
            }
            ...
            if let Some(ref mut s) = stream {
                let buf = match bincode::serialize(&batch) {
                    Ok(b) => b,
                    Err(e) => { warn!("Serialize error: {}", e); batch.clear(); continue; }
                };
